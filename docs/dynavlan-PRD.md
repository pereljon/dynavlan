# dynavlan - Product Requirements Document (v3.5)

Status: Draft v3.5, design-locked and hardware-validated 2026-07-14; implemented 2026-07-21..23 with three review rounds folded in; corrected against live hardware 2026-07-25; all-trunks redesign implemented 2026-07-29 (v0.2.0). Not yet released.
...
- FR-21 [HIGH / CONTINUITY]: Timer rescan (`dynavlan --rescan`, all-trunks, v3.5): strictly add-only, run per trunk over every currently detected trunk, then combined into one apply. Newly detected, in-range, non-ignored, unexcluded VLANs are added on whichever trunk carries them; nothing is ever removed. Do not re-DHCP a VLAN that already has an active lease.
- FR-22 [CRITICAL / CONTINUITY] (revised v3.5, all-trunks): Boot (`dynavlan --boot`) reconciles toward the detected set on EVERY carrier-up trunk with tags, looped over the union of currently-owned and currently-detected trunk interfaces. ADDITIONS are single-pass, computed per trunk. GUARD: if NO trunk at all is detected (no carrier anywhere, or no interface anywhere carries tags), ABORT the reconcile and change nothing. "Detected nothing" must never mean "remove everything." The `CARRIER_WAIT_SECONDS` expiry path (FR-2) feeds directly into this abort when it leaves every iface carrierless. A single trunk going dark while others stay live is NOT this guard - see FR-23's per-trunk preserve rule. (CONTINUITY not RECOVERABILITY because removal only affects dynavlan's own VLANs; the guard prevents a spurious monitoring outage + restart churn on an unplugged-at-boot.)
- FR-23 [HIGH / CONTINUITY] (revised v3.5, all-trunks): REMOVALS during boot reconcile require two-pass confirmation WITHIN the single `--boot` invocation, evaluated PER TRUNK: sniff, wait `BOOT_SETTLE_SECONDS`, sniff again, and on each trunk remove only that trunk's owned VLANs absent from BOTH passes on that trunk. A trunk with no carrier or empty detection in either pass has ALL its owned VLANs preserved for this boot (not removed) rather than counted as evidence of absence. This avoids tearing down a real-but-idle VLAN during the least-reliable window (STP/LLDP not converged at boot), and avoids destroying a trunk's whole owned set the moment it happens to be briefly dark. Rescan (FR-21) remains strictly add-only and never removes, on any trunk. Boot-time cost is roughly `CARRIER_WAIT + 2×SNIFF_SECONDS + BOOT_SETTLE_SECONDS` (see NFR-4) regardless of how many trunks are detected (detection cost is port-count-independent, FR-2).
- FR-24 [CRITICAL / RECOVERABILITY]: Removing a VLAN requires BOTH dropping its stanza from `90-dynavlan.yaml` AND explicitly `ip link delete <iface>.<id>` (netplan apply alone leaves the orphaned interface up, validated). ORDERING: the explicit `ip link delete` happens ONLY AFTER the single box-wide `netplan try` covering every trunk's change is ACCEPTED (FR-18 health check passed), never before, and never if the change reverted. If the add/remove change reverts, no interface is deleted on any trunk (else netplan's file-revert would not recreate the destroyed interface and monitoring would be silently lost).
...
- AC-3 [HIGH] (revised v3.5, all-trunks): A trunk whose VLAN set changes (switch-side reconfiguration) drops its stale VLANs (explicit delete, after accept) and configures the new ones on reboot, in a single reconcile, only when that trunk's detection is non-empty across both boot passes; a trunk that goes dark entirely (no carrier, or no tags) has its owned VLANs PRESERVED, not removed - there is no cross-trunk relocation. Superseded original wording (through v3.4): "a box moved to a different site drops the old VLANs and configures the new ones" described relocating one selected trunk's set onto a different NIC; that scenario no longer applies once every trunk is provisioned independently - a physical trunk move now just means the old NIC goes dark (preserved) while the new one is already, independently, provisioned on its own evidence.
