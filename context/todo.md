...
D1 script fix landed as ver 0.4.12 (FR-22 abort log level) and `--help`->0 as **ver 0.4.13**
(both rides-the-next-release, not tagged). All designed with an independent Fable-5 adversarial
reviewer. Gate (3) second-box validation is next (a candidate box exists); only C1
...
- [x] Decide `--help` exit code (done 2026-08-07): changed to `0`, requested help is success;
      no-mode/unknown stay `2`. ver 0.4.13, §1l-bis tripwires. (`context/open_questions.md`)
...
- [ ] Add a consolidated **Hardware Validation** section to `dynavlan/CLAUDE.md` (the rules are currently scattered across Testing Plan / Development Workflow / Commands / Version-and-build-identity). Fold in: console-backed only (never SSH-only), the FR-38 build-identity gate before drawing any conclusion, and a new rule to **snapshot/back up `/etc/dynavlan.conf` before any package upgrade on a box** (belt-and-suspenders over dpkg confold, so a modified conffile can always be diffed/restored). Point to `docs/hardware-validation-*.md` as the per-pass checklist. Raised 2026-08-06 during the v0.4.11 HV pass.
- [ ] (Deferred, from gate-4 M8) Persisted machine-readable last-run RESULT for `--status`. FR-35 originally promised it; M8 narrowed FR-35 to a `journalctl -t dynavlan` hint instead, since a `/run/dynavlan/last-run` record would add state + a write on the apply exit path for marginal gain over the journal. Revisit if a real consumer needs it without parsing the journal.

## Completed milestones

- [x] **v0.4.11 RELEASED (done 2026-08-06)**: consolidates the pre-1.0 gate-4 hardening (0.4.3->0.4.11).
      Consolidated hardware-validation pass PASSED 14/14 on the Protectli box, console-backed, build
      `b0317b6` (installed from a locally-built `.deb` over the prior 0.4.2 test build): build-identity
      gate; Phase 1 smoke + M8 dep-gate + M5 drain; H5 lease timing (7 leases in one 179ms bounded pass,
      not per-VLAN serial; restart fired once, run 76s << 600s timeout); M6 restart-failure seen-set
      (total failure holds subnet unseen + retries, partial success consumes + no storm); P2 kill-mid-
      `netplan try` FIFO-EOF revert (~4.3s after writer death, 14 ifaces intact, box reachable); L3-30
      carrier-pull PRUNE + re-add re-confirmed on 0.4.11. C1 POSITIVE refusal DEFERRED (needs switch to
      make a dynavlan VLAN the default route; true-negative observed live during the prune). 3b.2
      systemd-cgroup-kill variant not run (secondary). Sign-off: `docs/hardware-validation.md`.
      Docs updated (CHANGELOG `[0.4.11]`, SKELETON P2 EOF status, tests-doc 2026-08-06 run note, context
      index/todo). Released via a docs-only commit on top of `b0317b6` (shipped script byte-identical to
      the validated one; `ver=` unchanged - no bump for docs). Tagged `v0.4.11`, pushed, GitHub release
      on `pereljon/dynavlan`; Actions builds the `.deb` + updates the APT repo. Box still runs the 0.4.11
      test build; `apt upgrade` swaps in the packaged build when the operator is ready. Timer restored
      active; config at defaults.
