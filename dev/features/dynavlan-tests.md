...
**1x. `build-deb.sh` version binding (H7, FR-38):** the release job is tag-triggered and passes the tag-derived version to `build-deb.sh`; if it disagrees with the script's `ver=`, the package (and the `--version` it installs) would misreport the build. `1x` asserts `build-deb.sh 9.9.9` exits non-zero with a "does not match" message - keyed on the message, not a bare non-zero, because a dev box lacking `dpkg-deb` also exits non-zero and would mask a missing guard. The guard is placed before the `dpkg-deb` requirement so the mismatch is the reported failure and the test stays portable.

**1y. `apply_settle_floor` - C2 apply-evidence floor (minimal-#1):** a no-addition change (removal-only, `--reapply`) has no in-kernel apply signal during the netplan-try window, so `seen_at` anchors at t=0 and the box relies on a settle floor conservative enough that the first health sample lands post-apply, not on pre-apply routing. `1y` asserts the selection (probe present -> `APPLY_FALLBACK_SETTLE`; no probe -> the larger `APPLY_NOEVIDENCE_SETTLE`) AND two relationships rather than the bare value: the no-evidence floor exceeds the fallback floor, and it stays `<= TRY_TIMEOUT - 2*POLL_INTERVAL - HEALTH_CONSEC*POLL_INTERVAL` (= 22) so a no-addition change can still reach `HEALTH_CONSEC` PASSes before `accept_cutoff` (= 26). Keying on the relationships means tuning the constant on hardware fails the suite if the value is pushed past the cutoff, rather than silently making removals un-acceptable. **Layer-2 (hardware, VALIDATED 2026-08-05):** two no-addition `--reapply` applies (routed-on, isolation revert) both ACCEPTED at ~18.2s applying->ACCEPTED, AND a GENUINE carrier-down removal-only `--rescan` (operator toggled the enp1s0 switch port) removed 7 VLANs at a measured **18.19s** applying->ACCEPTED - exactly the floor-16 prediction (samples at 16s+18s, under the 26s cutoff), health passing on the post-prune default. `APPLY_NOEVIDENCE_SETTLE=16` finalized by measurement (apply completes well under 10s here; accept times are floor-dominated). See `context/decisions.md` 2026-08-05.
**1z. Config isolation - parse-only allowlist (H2):** `load_config` no longer `source`s `/etc/dynavlan.conf`; `config_load_file` parses it declaratively so a config line can never execute code or assign an internal/safety variable. `1z` asserts: `config_allowed_key` accepts each documented key and refuses internal names (`NETPLAN_FILE`, `TRY_TIMEOUT`, `ver`, `PATH`) and typos (`VLAN_MINN`); `config_normalize_value` strips inline comments and surrounding quotes while preserving inner and unquoted-list spaces (so `a b c` is not silently truncated to `a`); and `config_load_file` on real temp files assigns allowlisted keys, REFUSES a protected-name assignment with the global left untouched, refuses a non-assignment line, and - the load-bearing security assert - proves a `$(...)` in a value is an inert literal that creates no side effect (a sentinel file is never touched). The two claims that matter (protected global untouched, no code execution) are asserted directly, not inferred. **Layer-2 (hardware):** none needed - config parsing is pure and fully covered at Layer-1; there is no apply/network interaction to exercise on the box.
**1aa. `default_iface_in_removals` - C1 routed-mode delete guard:** in routed mode a dynavlan VLAN can be the box's default egress; if it is also in the removal set, the post-ACCEPT `ip link delete` would strand the box (health PASSes during the try because the VLAN is still up; the delete is unrevertable). `apply_change` refuses the whole reconcile before any disk change when this holds. `1aa` asserts the guard fires exactly when a NON-EMPTY default iface is an exact removal token, and NOT for the base uplink (isolation default, never a token), no default route, a non-removed default VLAN, or a same-id VLAN on a different trunk (`enp2s0.100` vs `enp1s0.100`); plus explicit substring/prefix-safety asserts (`.100` vs `.1000`, `.10` vs `.100`) that would catch a future `in_list` regression to glob/prefix matching. **Layer-2 (hardware, TRUE-NEGATIVE VALIDATED 2026-08-05; positive refusal DEFERRED):** the guard's real integration was exercised on a genuine carrier-down removal (operator toggled the enp1s0 switch port): at apply time snap_iface=enp2s0 (the default after enp1s0 dropped) and removals=enp1s0's VLANs, so `default_iface_in_removals` correctly returned false and the guard stayed SILENT - the legitimate removal of a non-default trunk proceeded (no false-fire). The dry-run preview was also confirmed correctly absent in the normal case. The faithful POSITIVE refusal (a routed VLAN that IS the box's default being removed) still needs switch access to set up - a wire-present VLAN is never a removal (`boot_removals` uses RAW detection, `dynavlan:1547,1607`), and forcing a VLAN to be the lowest-metric default needs a sub-uplink metric (`metric_conflict` refuses) or real uplink failover - so it remains deferred, backed by the full §1aa unit coverage + code review (placement before any disk change). See `context/decisions.md` 2026-08-05.

**1ab/1ac. `detection_sample_valid` + `store_detection` - H1 detector-failure guard:** a capture method that FAILS at runtime returns an empty/truncated id set indistinguishable from a genuinely quiet trunk, and a failure-truncated set must never authorize a boot detection-diff removal. `1ab` covers the pure verdict `detection_sample_valid METHOD SNIFF LLDP PREP → valid|""`: prep failure invalidates in every mode; `sniff`/`lldp` modes require their one method ok; and crucially under `both` only the primary SNIFF must have succeeded (`both ok fail ok → valid`, `both fail ok ok → ""`), because a failed sniff is the real removal hazard while LLDP is a non-fatal supplement (Option B, `context/decisions.md` 2026-08-05). `1ac` locks the string contract between `detect_iface` (emits `"SNIFFSTATUS LLDPSTATUS IDS..."`) and `store_detection` (parses it, stashes `TAGS_`+`VALID_`): the hazard case `store_detection v "fail ok 200"` yields `TAGS=200` (still usable for additions) but `VALID=invalid` (removal suppressed); a prep failure invalidates even with ids present; an empty raw line (missing scratch file) defaults to `invalid`; and a genuinely quiet-but-successful sample (`"ok ok"`) is `valid` with empty tags, so a real absence still drives the diff. **Not unit-testable:** the wire trigger (a real NIC promisc/tcpdump-open failure) cannot be reproduced on the Protectli box; the guard only ever makes the tool MORE conservative (preserve, not remove), so it fails toward no-change and needs less validation than a fix that authorizes new actions.

**1ad. `ifaces_without_carrier` - M4 addition-drop for an unowned trunk that dies before apply:** additions are computed from the pass-1 detection sample, before the carrier verdict. The existing `drop_iface_tokens` guard (1w) only fires for OWNED trunks on the carrier-down removal path, so a newly-detected but not-yet-owned trunk that loses carrier in the detection->apply window keeps its stanzas on a dead parent (never lease, stall `wait_leases`, pointless restart). `1ad` covers the pure-ish helper `ifaces_without_carrier ADDITION_TOKENS → ifaces lacking carrier NOW` (re-stubbing `has_carrier` via `STUB_UP`, the §1s pattern): one dead addition iface among live ones is reported, all-live yields none, all-dead reports both, empty input yields none; and an end-to-end assert folds its output into `drop_iface_tokens` to show the dead trunk's additions dropped and the live trunk's kept. do_boot/do_rescan fold it in UNCONDITIONALLY (regardless of `need_pass2`/ownership), and do_dryrun applies the same filter so the preview never over-lists an addition the real run would skip (same discipline as M3/H3 dry-run parity). **Not hardware-reproducible on the Protectli:** it needs a trunk to die in the narrow detection->apply window; the guard only ever drops additions (fail-toward-no-change, re-added next rescan), so unit + `--dry-run` coverage suffices, same class as H1/H5.

**1ae. pending-delete record + drain - M5 retry of a failed post-ACCEPT `ip link delete`:** the delete runs only after ACCEPT, at which point the stanza is already gone from our file; a transient failure would strand the live interface (and its subnet) until reboot, and the next run cannot tell that orphan from a genuine external VLAN (`backend_list_managed_vlans` excludes it). `1ae` points `PENDING_DELETE_FILE` at a temp file (not `/run`) and covers `pending_deletes` / `write_pending_deletes` / `record_pending_delete`: absent record yields empty, round-trip is sorted-unique, `record` appends and dedups, an empty write removes the file. The drain is exercised with `backend_remove_vlan` stubbed to fail for a token marked `stuck` and succeed otherwise: `drain_pending_deletes` keeps only the still-failing token, and a clean drain (everything deletes) clears the record entirely. `apply_change` records a failed delete; `drain_pending_deletes` runs at the start of every do_boot/do_rescan; do_dryrun previews the pending set read-only. Ownership-safe: only tokens dynavlan itself removed are ever recorded, so a retry never deletes an external VLAN. **Not hardware-reproducible on the Protectli:** it needs `ip link delete` to fail transiently; the record is ownership-safe and reboot-scoped, so unit coverage suffices, same class as M4.

**1af. restart-failure must not consume the new-subnet event - M6:** a TOTAL monitoring-target restart failure previously still wrote the subnet into the seen-set (`restart_targets` warns-and-continues per target; `maybe_restart_on_new_subnet` grew `seen` unconditionally), so the subnet was never retried and the agent never learned it this uptime. `1af` points `SEEN_FILE` at a temp file, stubs `current_subnets` to a fixed token, and stubs `snap`/`systemctl` so the REAL `restart_targets` and the seen-write branch both run. Five cases: a total failure (the only target fails) leaves the subnet unseen (retried next run); a successful restart records it as seen (regression guard); a PARTIAL failure (one of two targets restarts) CONSUMES it (total-failure only - a single broken target must not force a perpetual re-restart of the working one); a total failure across two targets leaves it unseen (the multi-target total path); and the `apply_change`-already-restarted dedup branch with `RESTART_NONE_SUCCEEDED_THIS_RUN=1` preset also leaves it unseen, covering a failed VLAN-driven restart. **Not hardware-reproducible on the Protectli:** it needs a snap/systemctl restart to fail transiently; the seen-set is reboot-scoped and the failure only defers consumption, so unit coverage suffices, same class as M5.

**1ag. `check_detect_deps` - M8 status/reconcile detection dependency gate:** `--status` previously returned before `check_preconditions`, so a missing tcpdump/lldpctl/netplan let its detection pass print `detected trunks: [none]` exit 0 - a confident, wrong "no trunk" indistinguishable from a quiet wire. The tool checks are extracted into `check_detect_deps` (netplan>=min + tcpdump/`-Q in`/lldpctl per DETECT_METHOD; NOT root or `netplan try`, which gate the apply, not a report), shared by `check_preconditions` and `do_status`. `1ag` stubs the stubbable seams (`have_cmd`, `netplan_version`, `tcpdump_supports_inbound`) so presence is deterministic regardless of the host: all-present passes; a missing tcpdump, missing lldpctl, tcpdump lacking `-Q in`, missing netplan, and a too-old netplan each fail; and the DETECT_METHOD gating is covered (lldp-only does not require tcpdump, sniff-only does not require lldpctl). **Not hardware-reproducible on the Protectli:** it needs a detection tool absent from an otherwise-working box; the gate is non-mutating and the failure only downgrades status to "unavailable" + non-zero, so unit coverage suffices.

**1ah. `ids_in_ignore` / `ids_out_of_range` - M8 per-trunk excluded/ignored (FR-35):** pure display helpers for `--status`, showing which detected VLANs the reconcile would exclude and why. `1ah` covers the intersection with `VLAN_IGNORE` and the out-of-`[MIN,MAX]`-range set, including empty-input cases; output is numeric-sorted and deduped via `emit_set`.
...
Exit code (M3, FR-34) [VALIDATED 2026-08-04 on the Protectli box]: `--dry-run` returns non-zero when validation FAILs, 0 otherwise. Confirm on a box: with a valid setup `sudo dynavlan --dry-run; echo $?` prints 0; introduce a validation failure (e.g. drop a deliberately malformed `/etc/netplan/99-broken.yaml`, which fails `netplan generate` as a base-file error) and confirm the run prints `validate: FAIL - a BASE netplan file is invalid` AND `$?` is non-zero; remove the broken file afterward. Not unit-testable in the pure suite (it needs a real `netplan generate` and a `/run` scratch tree); belongs to the future injected-command harness or this on-box check.
...
| L3-18 | Carrier-up tagless trunk, preserved not relocated (AC-3, all-trunks) | With owned VLANs on NIC A that stays carrier-UP but shows no tags in either boot pass, and NIC B now carrying tags; `--boot` | A's owned VLANs are PRESERVED (carrier-up + no tags in either pass = genuine detection uncertainty, skip removals on A, per-trunk); B is independently provisioned as ADDITIONS on its own trunk, in the SAME reconcile, alongside A's untouched set - there is no relocation branch moving A's set onto B. (A going CARRIER-DOWN is pruned per FR-41, not preserved - see L3-30, not this case.) | AC-3 |
...
| L3-37 [VALIDATED 2026-08-04] | Debian upgrade preserves operator timer state (H3) | On a dpkg-managed box, set each state, then `dpkg -i` the new build and check `systemctl is-enabled`/`is-active dynavlan.timer`. Fixed->fixed: (a) enabled+active, (b) enabled+stopped, (c) disabled+stopped. Transitional pre-fix(0.4.0)->fixed(0.4.1): (d) disabled+stopped, (e) enabled+stopped | PASS on the Protectli box (0.4.1 test build, console fallback up, no reconcile fired at any point). Fixed->fixed restored EXACTLY via the prerm->postinst state file: (a) enabled+active, (b) enabled+inactive, (c) disabled+inactive. Transitional (missing state file -> live `is-enabled` + `tmr_active` follows `tmr_enabled`): (d) disabled+stopped stayed **disabled+inactive** - a disabled timer is never started (this caught + fixed a bug where `tmr_active` defaulted to 1 unconditionally and started the disabled timer); (e) enabled+stopped came up **enabled+active** (preserve-running for an enabled timer, the one documented transitional imperfection, self-corrects on reboot). SEPARATE PRE-EXISTING FINDING (not H3): a *fresh* `dpkg -i`/`apt install` starts the timer via dpkg/systemd machinery (NOT postinst - proven by step-by-step repro), so first-install "never start" does not currently hold; logged in `context/open_questions.md` | AC-7 |
...
**Hardware run 2026-08-06 (v0.4.11 gate-4 consolidated pass, console-backed):** the accumulated 0.4.3->0.4.11 hardening validated on the Protectli box (build `b0317b6`), installed from a locally-built `.deb` over the prior 0.4.2 test build. Full sign-off matrix: `docs/hardware-validation-v0.4.10.md`. Headline results: build-identity gate PASS; M8 dep-gate (hidden `lldpctl` -> `detected trunks: unavailable`, exit 1); H5 lease timing (7 leases in one 179 ms bounded pass, not per-VLAN serial); M5 drain no-op; M6 restart-failure seen-set (total failure holds subnet unseen + retries, partial success consumes + no storm); P2 kill-mid-`netplan try` -> FIFO-EOF revert ~4.3s after writer death, box intact (validates the SKELETON "involuntary EOF" case); and L3-30 carrier-pull PRUNE + re-add re-confirmed on 0.4.11. C1 POSITIVE refusal (routed-mode default-route-in-removals) remains the one deferred case (needs switch access to make a dynavlan VLAN the box default route); its true-negative was observed live during the prune. Preserve-biased detail noted: neither `VLAN_MAX` nor `VLAN_IGNORE` prunes an already-owned VLAN, and `--reapply` no-ops on a matching on-disk yaml (both drove the P2 staging approach).
