...
| L3-37 | Debian upgrade preserves operator timer state (H3) | On a dpkg-managed box, run the four states before an `apt upgrade`/`dpkg -i` to a new build: (a) timer enabled+active (default), (b) `systemctl disable dynavlan.timer` (disabled, still-active until stop), (c) `systemctl stop dynavlan.timer` (enabled, inactive), (d) `systemctl disable --now dynavlan.timer` (disabled, inactive). Upgrade, then check `systemctl is-enabled`/`is-active dynavlan.timer` and `dynavlan.service` | Each state is restored EXACTLY: a disabled timer is NOT re-enabled and NOT started; a stopped-but-enabled timer is re-started (prior active state); a fully-disabled timer stays down. First install (purge then install) still enables both for next boot and starts nothing. Transitional note: upgrading FROM a pre-H3 package (whose prerm records no state) restores enablement from live `is-enabled` but re-starts a stopped-but-enabled timer, since the outgoing prerm cannot record the running state; validate this once against the last pre-fix release too | AC-7 |
