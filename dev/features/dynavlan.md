...
| `plan_route_metrics` / `map_ids` / `assign_route_metrics` / `map_filter` / `metric_conflict` | FR-37 routed mode (tests 1g): `plan_route_metrics` is the single assignment decision point every caller must use (owned map + target + additions -> full map; assigns for target ids LACKING a metric, not additions); pure metric assignment (`discovery` = kept verbatim + next-free for new; `id` = START+id stateless), map filtering to surviving ids, and the uplink-metric conflict guard (CONFLICT if any assigned metric <= uplink metric) |
