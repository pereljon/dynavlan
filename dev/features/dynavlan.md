...
| `do_boot` | Loops every owned-or-detected trunk; per-trunk two-pass math (additions single-pass, removals need absence in both passes on that trunk; zero-detection guard is box-wide - no trunk detected at all aborts). Removal-set combination per trunk factored into a pure helper `boot_removals(owned,pass1,pass2)` for unit testing (tests 1d). Combines every trunk's tagged additions/removals into one token set, then ONE unified `apply_change` for the whole box |
| `do_rescan` | Add-only, looped over every `DETECTED_TRUNKS` iface; combines into one token set, one unified apply |
...
**`--boot`** (FR-22/23, all-trunks): preconditions → discover/prep every physical iface → detect pass 1 → **box-wide guard: if `DETECTED_TRUNKS` is empty (no carrier anywhere / no tags anywhere), ABORT, change nothing** → loop over the UNION of every owned trunk and every newly-detected trunk; per trunk, additions = detected ∩ range − ignore − managed-elsewhere − owned-on-that-trunk, tagged to `iface.id` → sleep `BOOT_SETTLE_SECONDS` → detect pass 2 (re-run over all ifaces) → per trunk, **pass 2 counts for removals only if THAT trunk has carrier at pass-2 time** (an empty read off a dead link is not evidence of absence; otherwise skip removals on that trunk this boot, warn, preserve its owned VLANs) → removals on a trunk = owned VLANs on it absent from BOTH passes → **no relocation branch**: a trunk that goes dark (no carrier, or tagless in both passes) simply has its owned VLANs preserved; there is nothing to relocate to, since every other tagged trunk is already independently provisioned on its own merits, not competing for a single "selected" slot → combine every trunk's tagged additions/removals into one token set → count gate (`gate_vlan_count`: FR-12 warn / FR-36 refuse-or-fill, over the combined total) → if change: ONE unified `apply_change` for the whole box → done.

**`--rescan`** (FR-21, all-trunks): preconditions → discover/prep → detect (single pass) → loop every `DETECTED_TRUNKS` iface, per-trunk additions only (add-only), tagged and combined into one token set → if change: one unified `apply_change`.

**`--dry-run`** (FR-34, all-trunks): detect + per-trunk candidate math over the union of owned and detected trunks (single pass, boot-mode math) + `backend_validate` against a throwaway tree → print the combined intended add/remove plus a per-trunk breakdown; **never apply, never restart**. The throwaway tree is a copy of the real `/etc/netplan/` (base files included) with the candidate `90-dynavlan.yaml` overlaid, so `netplan generate` sees base files too and dry-run surfaces an FR-17 base-file-freeze (R-5) rather than hiding it. Lock interaction (round-4): dry-run TRIES the FR-30 flock non-blocking - if free it holds it for the preview (a timer rescan landing mid-preview skips normally and retries next cycle); if already held it does NOT block the operator, warns the preview may reflect mid-change state, and proceeds read-only.

**`--status`** (FR-35, all-trunks): print owned vs detected-now vs excluded/ignored for every owned trunk plus every currently-detected trunk, last-run result.
