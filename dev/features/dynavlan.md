...
| `have_routing` | FR-41 pre-condition (tests 1s): true iff a lowest-metric default route exists AND its egress iface has carrier. False → callers preserve rather than attempt a doomed removal (composition of existing `snapshot_default_route` + `has_carrier`) |
| `do_boot` | Loops every owned-or-detected trunk; per-trunk two-pass math (additions single-pass, detection-diff removals need absence in both passes on a carrier-UP trunk; hard `run_detection` failure aborts box-wide, an empty-but-successful detection does not). Removal-set combination per trunk factored into a pure helper `boot_removals(owned,pass1,pass2)` for unit testing (tests 1d). A second settle+resample (`need_pass2`) also fires when FR-41 carrier-down removal is armed and an owned trunk was already down at pass 1 (independent of `RESET_ON_BOOT`); on sustained two-sample carrier loss with healthy routing (`have_routing`), `carrier_removals` prunes the whole owned set on that trunk. Combines every trunk's tagged additions/removals into one token set, then ONE unified `apply_change` for the whole box |
...
FR-41 (carrier-down removal, §11b) adds one PRE-condition in front of this chain, never inside it: `have_routing()` gates whether a carrier-down trunk's VLANs are even proposed for removal (`all_removals`) before `apply_change` is called at all. The chain itself is unchanged - the existing health-check POST-condition (line 113 above) remains the only backstop that actually runs `apply_change`, so a carrier-down removal that somehow broke routing still reverts exactly like any other change.
...
**The rule.** When an OWNED trunk's link is carrier-down across a two-sample debounce (`BOOT_SETTLE_SECONDS` apart, reusing the boot pass-2 settle where one already runs) AND the box still has a healthy default route (`have_routing`), remove ALL owned VLANs on that trunk - a full teardown of the trunk's set, not a per-VLAN detection diff, since a dead port kills every VLAN riding it equally.
