...
Default is full isolation: every VLAN is DHCP address-only, `use-routes/use-dns/use-ntp/use-domains` all false (all four are required; anything less lets networkd pin DHCP-provided DNS/NTP as host routes), plus `accept-ra: false`. With `VLAN_ROUTES=true` the stanza flips to `use-routes: true` + `route-metric: N`; DNS/NTP/domains stay declined. The id:metric map is computed once per run by `plan_route_metrics`, the single decision point every caller must use (a second call site recomposing the pieces is how the two silently diverge): kept VLANs reuse the metric read back from the owned YAML itself (the file is the state store); the ids assigned a NEW metric are those in the target that do not already have one - NOT the additions. Those sets differ exactly when an owned VLAN has no persisted metric, which is what an isolated -> routed migration looks like, and assigning for additions alone left the rest unassigned and froze generation permanently (fixed 2026-07-25). Mode governs the new assignments (`discovery` = next free slot, existing never renumber; `id` = START + id, stateless). If any assigned metric would match or beat the uplink's, refuse before touching disk - applying would guarantee a health-FAIL revert loop.
