...
Default is full isolation: every VLAN is DHCP address-only, `use-routes/use-dns/use-ntp/use-domains` all false (all four are required; anything less lets networkd pin DHCP-provided DNS/NTP as host routes), plus `accept-ra: false`. With `VLAN_ROUTES=true` the stanza flips to `use-routes: true` + `route-metric: N`; DNS/NTP/domains stay declined. The id:metric map is computed once per run inside `apply_change`: kept VLANs reuse the metric read back from the owned YAML itself (the file is the state store); additions are assigned by mode (`discovery` = next free slot, existing never renumber; `id` = START + id, stateless). If any assigned metric would match or beat the uplink's, refuse before touching disk - applying would guarantee a health-FAIL revert loop.
